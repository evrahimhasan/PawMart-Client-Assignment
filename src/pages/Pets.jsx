...
const Pets = () => {
        ...
        const fetchListings = async () => {
            ...
            try {
                const res = await fetch(
                    `https://paw-mart-server-pi.vercel.app/listings?category=${category}&page=${page}&limit=${limit}`
                );
    ...
    return (
        ...
        <div className="space-y-12">
            <div className='flex justify-between items-center'>
                <select onChange={(e) => { setCategory(e.target.value); setPage(1); }} defaultValue="Chose category" className="select">
                    <option value="Chose category" disabled>Chose category</option>
                    <option value="">All</option>
                    <option value="Pets">Pets</option>
                    <option value="Foods">Foods</option>
                    <option value="Accessories">Accessories</option>
            ...
            <section className="text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {filteredListings.map((listing) => (
                        <div key={listing._id} className='p-6 rounded-xl shadow-md hover:-translate-y-2 transition'>
                            <div className="mb-5">
                                <img className="w-full h-[450px] object-cover rounded-lg" src={listing.image} alt="" />
                            </div>
                            <div className='px-6'>
                                <div className="flex justify-between font-semibold mt-3 text-xl">
                                    <h2 className="text-2xl font-bold">{listing.name}</h2>
                            ...
                            <Link
                                to={`/pets-details/${listing._id}`}
                                className="mt-3 inline-block bg-gradient-to-r from-orange-600 to-orange-800 text-white px-4 py-2 rounded-lg hover:bg-green-700"
                            >
                                View Details
                            </Link>
                        </div>
